...
public class EZScreenPlacement : MonoBehaviour, IUseCamera
{
	...
	public Vector3 ScreenPosToWorldPos(Vector3 screenPos)
	{
		...
		if (TBFUtils.GetUITextScaling() < 1f)
		{
			if (relativeTo.horizontal != HORIZONTAL_ALIGN.SCREEN_PERCENTAGE)
			{
				position.x *= TBFUtils.GetInverseUITextScaling();
			}
			if (relativeTo.vertical != VERTICAL_ALIGN.SCREEN_PERCENTAGE)
			{
				position.y *= TBFUtils.GetInverseUITextScaling();
			}
		}
		...
		case HORIZONTAL_ALIGN.OBJECT:
			...
			break;
		case HORIZONTAL_ALIGN.SCREEN_PERCENTAGE:
			position.x = renderCamera.pixelWidth / 100f * screenPos.x;
			break;
		...
		case VERTICAL_ALIGN.OBJECT:
			...
			break;
		case VERTICAL_ALIGN.SCREEN_PERCENTAGE:
			position.y = renderCamera.pixelHeight / 100f * screenPos.y;
			break;
