...
public class UIManager : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if (pointerType == POINTER_TYPE.AUTO_TOUCHPAD && !Application.isMobilePlatform)
		{
	...
	protected void PollMouseAndTouchpad()
	{
		...
		if (numPointers > 0)
		{
			PollMouse(ref pointers[0, numPointers - 1]);
			for (int i = 1; i < uiCameras.Length; i++)
			{
				if (uiCameras[i].camera == null)
				{
				...
				{
					pointers[i, numPointers - 1].Reuse(pointers[0, numPointers - 1]);
					pointers[i, numPointers - 1].prevRay = pointers[i, numPointers - 1].ray;
					pointers[i, numPointers - 1].ray = uiCameras[i].camera.ScreenPointToRay(pointers[i, numPointers - 1].devicePos);
				}
			}
		}
	}
