...
public class UIPanelManager : MonoBehaviour, IEZDragDrop, IUIContainer, IUIObject
{
	...
	public delegate void EZPanelChangedDelegate(UIPanelBase newPanel);
	...
	protected EZPanelChangedDelegate panelChangedDelegate;
	...
	private void SetCurrentPanel(UIPanelBase newPanel)
	{
		if (curPanel != newPanel)
		{
			curPanel = newPanel;
			if (panelChangedDelegate != null)
			{
				panelChangedDelegate(curPanel);
			}
		}
	}

	public virtual void AddPanelChangedDelegate(EZPanelChangedDelegate del)
	{
		panelChangedDelegate = (EZPanelChangedDelegate)Delegate.Combine(panelChangedDelegate, del);
	}

	public virtual void RemovePanelChangedDelegate(EZPanelChangedDelegate del)
	{
		panelChangedDelegate = (EZPanelChangedDelegate)Delegate.Remove(panelChangedDelegate, del);
	}
	...
	{
		Debug.Log("DecrementTransitioningPanels: " + p.name);
	...
	public bool MoveBack()
	{
		if (!linearNavigation)
		{
			...
			if (breadcrumbs.Count > 0)
			{
				SetCurrentPanel(breadcrumbs[breadcrumbs.Count - 1]);
			}
			if (curPanel != null)
			{
				if (deactivateAllButInitialAtStart && !curPanel.gameObject.active)
