...
public static class Language
{
	...
	public static void UpdateLanguageFromBedrock()
	{
		...
		foreach (string key2 in currentEntrySheets.Keys)
		{
			Hashtable hashtable = currentEntrySheets[key2];
			...
			foreach (string item in list)
			{
				...
				if (!Bedrock.GetRemoteUserResources(item, out resourceDictionary))
				{
					continue;
				}
				TBFUtils.DebugLog("Remote resource found for " + item);
				if (resourceDictionary != null)
				{
					string key = currentLanguage.ToString();
					if (resourceDictionary.ContainsKey(key))
					{
						TBFUtils.DebugLog("Replacing string " + item + " with copy from SWRVE");
						hashtable[item] = resourceDictionary[key];
					}
				}
				else
				{
					TBFUtils.DebugLog("Bedrock.GetRmoteUserResources succeeded, but stringResources not found for " + item);
				}
			}
		}
	}
