...
public class LocalizedAsset : MonoBehaviour
{
	...
	public static void LocalizeAsset(Object target)
	{
		...
		else if (target.GetType() == typeof(LegacyGUITexture))
		{
			LegacyGUITexture legacyGUITexture = (LegacyGUITexture)target;
			if (legacyGUITexture.texture != null)
			{
				Texture texture = (Texture)Language.GetAsset(legacyGUITexture.texture.name);
				if (texture != null)
				{
					legacyGUITexture.texture = texture;
				}
			}
		}
